...
export function ProjectStory({ project, index }: { project: Project; index: number }) {
  ...
  return <article className={`project-story project-${project.id} ${index % 2 ? 'story-reversed' : ''}`} aria-labelledby={`${project.id}-title`}>
    <div className="story-intro">
      ...
      <div className="story-narrative"><RevealHeading as="h4">{project.story.headline}</RevealHeading><p>{project.story.purpose}</p></div>
    </div>
